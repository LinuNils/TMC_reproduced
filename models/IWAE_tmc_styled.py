...
class IWAE:
    def __init__(self, input_shape, batch_size, layer_specs, k_samples, lr, sess, small):
        self.data_ph = tf.placeholder(dtype=tf.float64, shape=(None, k_samples, input_shape))
        self.train_ph = tf.placeholder(dtype=tf.bool)
        self.tot_obj_loss = tf.placeholder(dtype=tf.float64)
        ...
        self.k = k
        self.log_w = tf.zeros(dtype=tf.float64, shape=[batch_size, self.k])
    ...
    def dense(self, x_, num_units, init_scale=0.01, scope_name=''):
        ...
        with tf.variable_scope(scope_name):
            ...
            if self.init is not False:
                V = tf.get_variable('V', shape=[int(x_.get_shape()[-1]), num_units], dtype=tf.float64,
                                      initializer=tf.random_normal_initializer(0, 0.05), trainable=True)

                g = tf.get_variable('g', shape=[num_units], dtype=tf.float64,
                                      initializer=tf.constant_initializer(1.), trainable=True)

                b = tf.get_variable('b', shape=[num_units], dtype=tf.float64,
    ...
    def sample_z(self, nu, rho, value=None, bern=False):
        ...
        if bern:
            sample_dist = tf.distributions.Bernoulli(logits=nu, dtype=tf.float64)
        ...
        eps = tf.random_normal(tf.shape(nu), dtype=tf.float64)
        z_next = nu + rho * eps
        if value is not None:
            estimate = value
        else:
            estimate = z_next
        estimate_ = tf.reshape(tf.expand_dims(estimate, axis=-1),
                               [-1, 1, estimate.get_shape()[1], estimate.get_shape()[-1]])
    ...
    def objective_function(self):
        ...
        """
        k = tf.constant(self.k, dtype=tf.float64)
    ...
    def model(self, q_z_next):
        """
        IWAE model structure with sampling mechanism as implemented by the author of https://arxiv.org/pdf/1806.08593.pdf
        for the Non-facturized case
        ...
        for p_out, mlp_units, q_z_in, q_z_out in zip([8, 16, 32, 64, 784],
            ...
            with tf.name_scope('P_stochastic_layer'):
                ...
                if bern:
                    recon = p_z_next
            lP.append(p_logprob)
        lQ = [tf.matrix_diag_part(lq) for lq in lQ[::-1]]
        lP = [tf.matrix_diag_part(lp) for lp in lP]

        lQ = sum(lQ)
        lP = sum(lP)
        with tf.name_scope('log_w'):
            self.calc_logw(lQ, lP)
        return recon
...
k = args.k
epochs = args.epochs
save_path = 'IWAE_model_tmc_styled_non_fac_{}_k_{}'.format(model_type, k)
    ...
    try:
        for epoch in range(1, epochs+1):
            ...
            if epoch == epochs:
                ...
                saver.save(sess,
                           save_path+"/model_IWAE_tmc_styled_forward_non_fac_{}_with_k{}.ckpt".format(model_type, k))
        print(test_err)
