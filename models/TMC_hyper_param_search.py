...
parser = ArgumentParser("Hyper parameter search for TMC-paper from NeurIPS 2019")
parser.add_argument('-k',           dest='k',          type=int, default=20,    help="Option for choosing k")
parser.add_argument('--epochs',     dest='epochs',     type=int, default=500, help="Option for choosing number of epochs")
...
parser.add_argument('--learning_rate', dest='learning_rate', type=float, default=1e-2, help="Option for using small or large model")
...
k = args.k
epochs = args.epochs
save_path = 'TMC_model_hyp_param_srch_non_fac_{}_k_{}_learn_rate_{}'.format(model_type, k, lr)
    ...
    try:
        for epoch in range(1, epochs+1):
            ...
            if epoch == epochs:
                ...
                np.save(save_path+"/TMC_hyp_param_srch_tot_obj_loss_k_{}_non_fac_{}_lr_{}".format(k, model_type, lr), total_obj_loss_model)
                saver.save(sess,
                           save_path+"/model_TMC_hyper_param_srch_non_fac_{}_with_k{}_lr_{}.ckpt".format(model_type, k, lr))
        print(test_err)
        ...
        print('Stopped training and testing at epoch {} for model TMC non-factorized {} with k: {}'.format(epoch,
                                                                                                            ...
                                                                                                            k))
        total_obj_loss_model = np.array(test_err)
        np.save(save_path + "/TMC_hyp_param_srch_tot_obj_loss_k_{}_non_fac_{}_lr_{}".format(k, model_type, lr),
                total_obj_loss_model)
        saver.save(sess,
                   save_path + "/model_TMC_hyper_param_srch_non_fac_{}_with_k{}_lr_{}.ckpt".format(model_type, k, lr))
